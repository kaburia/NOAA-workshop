...
def extract(request):
    ...
    if extract_type in ['point', 'area_average', 'domain']:
        if extract_type == 'point':
            ...
            lon = np.float64(request['longitude'])
            lat = np.float64(request['latitude'])
            ...
            N = np.float64(request['N'])
            S = np.float64(request['S'])
            W = np.float64(request['W'])
            E = np.float64(request['E'])
        ...
        if np.float64(version) in allowed_versions:
            pass
        else:
